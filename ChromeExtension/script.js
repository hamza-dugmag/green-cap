...
const setDOMInfo = info => {
    document.getElementById('name').innerText = info.msg.substring(0, 35) + "...";  // product title
    
    if (info.msg.substring(0, 5) == "Otter" || info.msg.substring(0, 5) == "Spige" || info.msg.substring(0, 5) == "LK iP") {
      document.getElementById("rating").src = images[randint(3, 4)];  // high rating if one of the good ones
    }
    else {
        document.getElementById("rating").src = images[randint(0, 2)];  // bad rating if one of the bad ones
